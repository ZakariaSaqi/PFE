...
$month = [];
$data = [];
$req = "SELECT DATE_FORMAT(date_demande, '%b') AS month, COUNT(*) AS total_demande FROM demande GROUP BY month ORDER BY DATE_FORMAT(date_demande, '%m') ASC";
$res = $pdo->query($req);
while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
    $data[$row['month']]['demandes'] = $row['total_demande'];
}
$req2 = "SELECT DATE_FORMAT(date_don, '%b') AS month, COUNT(*) AS total_dons FROM don GROUP BY month ORDER BY DATE_FORMAT(date_don, '%m') ASC";
$res2 = $pdo->query($req2);
